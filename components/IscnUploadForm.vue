...
export default class IscnUploadForm extends Vue {
  ...
  async sendArweaveFeeTx(record: any, memoIpfs?: string): Promise<string> {
    ...
    const memo = JSON.stringify({ ipfs: memoIpfs || record.ipfsHash, fileSize: record.fileBlob?.size || 0 });
    try {
